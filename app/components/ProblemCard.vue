<template>
  <li class="pb-3">
    <v-card :color="difficultyColor[problem.difficulty]" raised class="black--text">
      <v-layout>
        <v-flex xs3>
          <v-card-title primary-title class="pl-3 py-2 pr-0">
            <div>
              <div class="title pb-1">No.{{ problem.num }}</div>
        ...
        <v-flex xs5>
          <v-card-title primary-title class="pl-3 py-2">
            <div>
              <div class="subheading">{{ difficulty[problem.difficulty] }}</div>
        ...
        <v-flex xs4 pl-0>
          <div class="py-0">
            <v-switch pt-0 color="primary" v-model="switch1" :label="labelText"></v-switch>
            <!-- <v-checkbox @click="addSentProblem(problem)" label="完登"></v-checkbox> -->
          </div>
        </v-flex>
      </v-layout>
      <v-divider pb-3 light></v-divider>
    </v-card>
  </li>
</template>
